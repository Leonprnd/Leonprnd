...
export default function Tijdlijn() {
  const { momenten, momentenGeladen, kaart } = useApp();
  ...
  const groepen = useMemo(() => {
    ...
    gesorteerd.forEach((moment) => {
      ...
      if (!perMaand.has(sleutel)) {
        perMaand.set(sleutel, { titel: d ? maandJaar(d) : 'Zonder datum', data: [] });
      }
  ...
  if (momentenGeladen && momenten.length === 0) {
    return (
      <View style={[stijl.vol, stijl.midden, { paddingTop: rand.top }]}>
        <Leeg
          icoon="📖"
          titel="Jullie verhaal begint hier"
          tekst="Zet het eerste plekje op de kaart, dan komt het hier te staan."
        >
          <Knop
            titel="Naar de kaart"
            icoon="🗺️"
      ...
      <SectionList
        ...
        ListHeaderComponent={
          <View style={stijl.kop}>
            <Titel>Ons verhaal</Titel>
            <Lopend zacht klein style={{ marginTop: 2 }}>
              {momenten.length} {momenten.length === 1 ? 'plekje' : 'plekjes'}
              {aantalFotos > 0
                ? ` · ${aantalFotos} ${aantalFotos === 1 ? 'foto' : "foto's"}`
                : ''}
            </Lopend>

            <View style={stijl.chipRij}>
              {mijlpalen > 0 ? (
                <Chip tekst={`${mijlpalen} mijlpalen`} icoon="✨" kleur={kleuren.goud} klein />
              ) : null}
              {eerste ? (
                <Chip
                  tekst={`vanaf ${korteDatum(eerste.datum)}`}
                  icoon="🌱"
                  kleur={kleuren.mint}
                  klein
                />
