...
import { Lopend, Chip, Bolletje, Leeg, Knop } from '../../src/components/basis';
...
export default function MomentDetail() {
  ...
  const { momenten, momentenGeladen, code, uid, ik, partner } = useApp();
    ...
    return (
      <View style={[stijl.vol, stijl.midden]}>
        <Leeg
          ...
          titel={momentenGeladen ? 'Dit plekje is er niet meer' : 'Even laden...'}
          tekst={
            momentenGeladen
              ? 'Misschien heeft je liefje het net weggehaald.'
              : undefined
          }
        >
          {momentenGeladen ? (
            <Knop
              titel="Terug naar de kaart"
              soort="zacht"
  ...
  const vanMij = moment.doorUid === uid;
  const maker = vanMij ? ik : partner;
      ...
      >
        ...
        >
          {fotos.length ? (
            ...
            <View style={stijl.geenFotos}>
              ...
              <Text style={stijl.geenFotosTekst}>Nog geen foto’s bij dit plekje</Text>
              <Pressable
                onPress={() =>
                  router.push({ pathname: '/moment/nieuw', params: { bewerk: moment.id } })
                }
              >
                <Text style={stijl.geenFotosKnop}>Foto’s toevoegen</Text>
              </Pressable>
            </View>
          )}
        </LinearGradient>
        ...
        <View style={stijl.inhoud}>
          <View style={stijl.chipRij}>
            <Chip tekst={type.label} icoon={type.icoon} kleur={type.kleur} />
            {type.bijzonder ? (
              <Chip tekst="Mijlpaal" icoon="✨" kleur={kleuren.goud} />
            ) : null}
          </View>
          ...
          {/* --- Wie en hartjes --- */}
          <View style={[stijl.makerRij, schaduw.zacht]}>
            <Bolletje emoji={maker?.emoji} kleur={maker?.kleur} maat={38} />
            <View style={{ flex: 1 }}>
              <Text style={stijl.makerNaam}>
                {vanMij ? 'Jij zette dit op de kaart' : `${maker?.naam || 'Je liefje'} zette dit op de kaart`}
              </Text>
              <Text style={stijl.makerOnder}>
                {partnerHartje && partner
                  ? `${partner.naam} vindt dit lief 💗`
                  : vanMij
                    ? 'Benieuwd wat je liefje ervan vindt'
                    : 'Geef er een hartje aan'}
              </Text>
            </View>
            ...
            >
              <Text style={stijl.hartTeken}>{mijnHartje ? '💗' : '🤍'}</Text>
            </Pressable>
          </View>
