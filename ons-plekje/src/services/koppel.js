...
export async function maakKaart({ uid, naam, emoji, kleur }) {
  ...
  throw new Error('Het lukt even niet om een code te maken. Probeer het nog eens.');
}
...
export async function doeMee({ code, uid, naam, emoji, kleur }) {
  ...
  if (!snap.exists()) {
    throw new Error('Deze code kennen we niet. Check of je hem goed hebt overgetypt.');
  }
  ...
  if (ids.length >= 2) {
    throw new Error('Deze kaart heeft al twee mensen. Vraag je liefje om een nieuwe code.');
  }
