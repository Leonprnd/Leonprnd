...
export default function DatumKiezer({ datum, opWijziging, style }) {
  ...
  return (
    <>
      ...
      >
        <View style={{ flex: 1 }}>
          <Text style={stijl.label}>Wanneer</Text>
          <Text style={stijl.waarde}>{langeDatum(datum) || 'kies een datum'}</Text>
        </View>
...
function Kalender({ open, datum, opKiezen, opSluiten }) {
        ...
        <Pressable style={[stijl.venster, schaduw.kaart]} onPress={(e) => e.stopPropagation()}>
          <View style={stijl.kop}>
            ...
            <Pressable onPress={() => setJaarLijst((x) => !x)} hitSlop={8}>
              <Text style={stijl.maandTitel}>
                {MAANDEN[bladert.maand]} {bladert.jaar}
              </Text>
              <Text style={stijl.maandHint}>{jaarLijst ? 'kies een jaar' : 'tik voor jaar'}</Text>
            </Pressable>
          ...
          <View style={stijl.onderrij}>
            <Pressable onPress={() => opKiezen(vandaag)} hitSlop={8}>
              <Text style={stijl.snelknop}>Vandaag</Text>
            </Pressable>
            <Pressable onPress={opSluiten} hitSlop={8}>
              <Text style={[stijl.snelknop, { color: kleuren.inktZacht }]}>Sluiten</Text>
            </Pressable>
          </View>
        </Pressable>
      </Pressable>
